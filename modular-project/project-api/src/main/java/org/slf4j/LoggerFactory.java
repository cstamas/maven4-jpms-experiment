...
public final class LoggerFactory {
    ...
    static final String UNSUCCESSFUL_INIT_MSG =
            "org.slf4j.LoggerFactory in failed state. Original exception was thrown EARLIER. See also "
    ...
    private static ServiceLoader<SLF4JServiceProvider> getServiceLoader(final ClassLoader classLoaderOfLoggerFactory) {
        ...
        if (securityManager == null) {
            ...
            final PrivilegedAction<ServiceLoader<SLF4JServiceProvider>> action =
                    () -> ServiceLoader.load(SLF4JServiceProvider.class, classLoaderOfLoggerFactory);
            serviceLoader = AccessController.doPrivileged(action);
        }
        return serviceLoader;
    }

    private static void safelyInstantiate(
            List<SLF4JServiceProvider> providerList, Iterator<SLF4JServiceProvider> iterator) {
        try {
    ...
    static SLF4JServiceProvider loadExplicitlySpecified(ClassLoader classLoader) {
        ...
        try {
            String message = String.format(
                    ...
                    explicitlySpecified, PROVIDER_PROPERTY_KEY);
            Util.report(message);
            ...
            String message =
                    String.format("Failed to instantiate the specified SLF4JServiceProvider (%s)", explicitlySpecified);
    ...
    private static void fixSubstituteLoggers() {
        synchronized (SUBST_PROVIDER) {
            ...
            for (SubstituteLogger substLogger :
                    SUBST_PROVIDER.getSubstituteLoggerFactory().getLoggers()) {
    ...
    private static void replayEvents() {
        final LinkedBlockingQueue<SubstituteLoggingEvent> queue =
                SUBST_PROVIDER.getSubstituteLoggerFactory().getEventQueue();
